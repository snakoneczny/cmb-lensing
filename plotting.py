...
def plot_cmb_imgs(data, img_col='image', n_img=80):
    ...
    for i in range(n_img):
        ...
        ax.imshow(data.iloc[i][img_col])  # extent
        ...
        if 'M200b' in data.columns:
            title += ', M200b: {:.4f}, M500c: {:.4f}, z: {:.4f}'.format(data.iloc[i]['M200b'], data.iloc[i]['M500c'],
                                                                        data.iloc[i]['z_halo'])
        ax.set_title(title)
